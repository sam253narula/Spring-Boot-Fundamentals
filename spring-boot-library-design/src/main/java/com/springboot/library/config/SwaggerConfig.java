...
//				.select().apis(RequestHandlerSelectors.basePackage("com.springboot.library.controller"))
//				.build();
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("Library-API").select()
				.apis(RequestHandlerSelectors.basePackage("com.springboot.library.controller")).build()
